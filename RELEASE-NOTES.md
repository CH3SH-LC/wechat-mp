...
2. 风格真正落地：8 套风格主题取自知识库色板（日系/国潮/校园/科技/极简/商务/手账/森系）——
   UI 显式选择优先，或 AI 按内容自动判断并在正文声明 `[[theme:名称]]`；渲染引擎按主题
   替换正文底色、标题/点缀/组件配色，素材配色同步贴合；风格不再是摆设。
3. 文案克制 + 组件必用：AI 产出按"文案语气规则"（禁用夸张营销词、感叹号 ≤1、具体细节、
   平实号召）与"结构规则"（正文必须组件化：容器 ≥2 + 提示气泡 ≥1 + 列表/引用 ≥1，
   纯文字段落 ≤2 连段）；引擎对组件化不足给出提示。
4. 强制询问：创作请求但关键信息不足（类型/风格/字数/调性/配图缺 ≥2 且未说"直接写"）时，
   AI 先在对话流里问 1 个精简问题、绝不直接产出，回答后再创作；闲聊与已清晰请求不受影响。
...
7. 模型自主对话：闲聊、公众号写作答疑由模型自主判断；无卡片、无按钮框。
