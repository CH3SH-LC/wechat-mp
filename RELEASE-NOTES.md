...
3. 对话流净化：气泡只显示 AI 说明文字，**不显示代码**；生成的 HTML 自动进入
   右侧预览；每条助手消息可点「查看 HTML 源码」展开（默认收起）。
4. 实时预览：回复中的 HTML 实时渲染为 375px 手机壳（可缩放 100-150%）。
5. 质量检查：v10 铁律客户端检查（零 emoji/零图标字符、无 style/script/文档级标签、
   无 linear-gradient/box-shadow、无外链图、无硬性宽度），通过/问题清单展示。
6. 导出：复制 HTML / 导出文件到 `文档/wechat-mp-workspace/exports/`。
7. 知识库懒加载：主包 <250KB（gzip <80KB），149 条目按需加载。
