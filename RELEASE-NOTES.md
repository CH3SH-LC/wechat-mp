...
2. v2 排版引擎（移植 DSH 完整工艺）：AI 创作时输出 **v2 语法正文**（Markdown + `[[banner]]`/`[[title]]`/`[[badge]]`/
   ...
   预览区提示转换器警告（art:///本地图/表格/超长）。真实模型实测：日系风 900 字正文全语法渲染、0 警告。
3. 模型自主对话：闲聊、公众号写作答疑、创作请求全部由模型自主判断——信息不足时 AI 在对话流里
   自然反问（最多 1 个关键问题），回答后直接产出；说"算了"停止创作回对话；无卡片、无按钮框、
   无本地规则路由。
4. 多会话上下文（DSH 式）：**左侧常驻会话栏**——每个会话独立对话历史与内容，
...
5. AI 创作生成：模式（自动 / 文字类 / 宣传类）+ 风格（校园/科技/国潮/日系/极简/商务/手账）
   选择随请求注入并参与知识检索；DeepSeek SSE 流式输出（含推理）。
6. 对话流净化：气泡只显示 AI 说明文字，**不显示代码**；生成的 HTML 自动进入
   右侧预览；每条助手消息可点「查看正文」/「查看 HTML 源码」展开（默认收起）。
7. 实时预览：v2 正文与 HTML 实时渲染为 375px 手机壳（可缩放 100-150%）。
8. 质量检查：v10 铁律客户端检查（零 emoji/零图标字符、无 style/script/文档级标签、
   无 linear-gradient/box-shadow、无外链图、无硬性宽度），通过/问题清单展示。
9. 导出：复制 HTML / 导出文件到 `文档/wechat-mp-workspace/exports/`。
10. 知识库懒加载：主包 <260KB（gzip <82KB），149 条目按需加载。
