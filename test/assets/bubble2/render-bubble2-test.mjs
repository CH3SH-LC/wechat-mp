// test/assets/bubble2/render-bubble2-test.mjs
// 重点气泡（module-bubble）资产 第二批 10 个（bubble2）：
//   渲染 4x PNG + probe 像素验证 + 体积 ≤1MB + zone 洁净度扫描 + 分组预览页
// 分组按"组合方式 × 风格"模型：同骨架多风格（①×3 风格 / ②×2 风格）突出类型×风格
// 输出：test/assets/bubble2/png-bubble2/<name>.png + preview-bubble2.html/.png
import { mkdirSync, writeFileSync, readdirSync, existsSync } from 'node:fs'
import { fileURLToPath } from 'node:url'
import { bubbleArts2, validateBubbleArts2 } from './ARTS-bubble2.mjs'
...
const outDir = fileURLToPath(new URL('./png-bubble2/', import.meta.url))
...
try {
  ...
  const group = (title, names) => '<div style="margin:0 0 24px"><h3 style="font-size:15px;color:#2f3640;margin:8px 12px">' + title + '</h3>' +
    names.map((n) => { const a = bubbleArts2.find((x) => x.name === n); return card(b64map[a.name], a) }).join('') + '</div>'
  ...
  const preview = '<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{margin:0;background:#f7f7f5;padding:16px;font-family:sans-serif}h2{font-size:18px;color:#2f3640;margin:8px 12px}p.note{font-size:12px;color:#888;margin:4px 12px 16px}</style></head><body>' +
    '<h2>重点气泡美术资产 · bubble2（10 个 · 类型 × 风格 · 整图资产 + 中央嵌字）</h2>' +
    '<p class="note">类型 = 7 种组合方式骨架；风格 = 图案词汇表（花草/球类/印章/几何）。画布 750×215（显示 ≈345×99），中央留空 zone（单泡 620×120 设计 / 双泡每泡 615×74）供文字叠放。三层验证：probe 像素 + zone 洁净扫描 + 375px 壳叠字 DOM 实测。</p>' +
    GROUPS.map(([t, ns]) => group(t, ns)).join('') +
    '</body></html>'
  const previewPath = fileURLToPath(new URL('./preview-bubble2.html', import.meta.url))
  ...
  await pv.screenshot({ path: fileURLToPath(new URL('./preview-bubble2.png', import.meta.url)), fullPage: true })
  await pv.close()
  ...
  console.log('PNG 输出: test/assets/bubble2/png-bubble2/（' + results.length + ' 张，4x）+ preview-bubble2.html/.png')
