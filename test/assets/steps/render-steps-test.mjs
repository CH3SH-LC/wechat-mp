// test/assets/steps/render-steps-test.mjs
// 步骤条（module-steps）资产 10 个：
//   渲染 4x PNG + probe 像素验证 + 体积 ≤1MB + zone 洁净度扫描 + 分组预览页
// 分组按"组合方式 × 风格"模型：4 种组合方式 × 多风格（突出类型×风格）
// 输出：test/assets/steps/png-steps/<name>.png + preview-steps.html/.png
import { mkdirSync, writeFileSync, readdirSync, existsSync } from 'node:fs'
import { fileURLToPath } from 'node:url'
import { stepsArts, validateStepsArts } from './ARTS-steps.mjs'
...
const GROUPS = [
  ['① 单色数字圆式（同骨架 × 3 风格）', ['steps-single', 'steps-campus', 'steps-minimal']],
  ['② 竖线贯穿式（同骨架 × 2 风格）', ['steps-vertical', 'steps-guochao']],
  ['③ 纯色圆+文字卡式（同骨架 × 3 风格）', ['steps-card', 'steps-tech', 'steps-promo']],
  ['④ 前提行+步骤组式（同骨架 × 2 风格）', ['steps-prep', 'steps-single-guochao']],
]

const outDir = fileURLToPath(new URL('./png-steps/', import.meta.url))
...
try {
  ...
  for (const a of stepsArts) {
    ...
    const r = await page.evaluate(async (art) => {
      ...
      // —— zone 洁净度扫描（§5.0 ②）：x105..730 区域逐行扫，相邻像素 RGB 差之和 >40 计数 ——
      const X0 = 105 * S, X1 = 730 * S
      const zoneMutations = []
  ...
  const group = (title, names) => '<div style="margin:0 0 24px"><h3 style="font-size:15px;color:#2f3640;margin:8px 12px">' + title + '</h3>' +
    names.map((n) => { const a = stepsArts.find((x) => x.name === n); return card(b64map[a.name], a) }).join('') + '</div>'
  ...
  const preview = '<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{margin:0;background:#f7f7f5;padding:16px;font-family:sans-serif}h2{font-size:18px;color:#2f3640;margin:8px 12px}p.note{font-size:12px;color:#888;margin:4px 12px 16px}</style></head><body>' +
    '<h2>步骤条美术资产 · steps（10 个 · 类型 × 风格 · 整图资产 + 右侧嵌字）</h2>' +
    '<p class="note">类型 = 4 种组合方式骨架（单色数字圆/竖线贯穿/纯色圆+文字卡/前提行+步骤组）；风格 = 图案词汇表（草叶/花枝/球类缝线/回纹/网格/极简）。画布：纵向 4 步条 750×220（显示 ≈345×101）/ 单步横向卡 750×120（显示 ≈345×55），右侧 x 130..730 留文字 zone（每步一行 ≤15 字）。三层验证：probe 像素 + zone 洁净扫描 + 375px 壳叠字 DOM 实测。</p>' +
    GROUPS.map(([t, ns]) => group(t, ns)).join('') +
    '</body></html>'
  const previewPath = fileURLToPath(new URL('./preview-steps.html', import.meta.url))
  ...
  await pv.screenshot({ path: fileURLToPath(new URL('./preview-steps.png', import.meta.url)), fullPage: true })
  await pv.close()
  ...
  console.log('PNG 输出: test/assets/steps/png-steps/（' + results.length + ' 张，4x）+ preview-steps.html/.png')
